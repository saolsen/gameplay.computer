...
import { GameKind, PlayerKind, Status, StatusKind } from "../gameplay/mod.ts";
import { NameSchema } from "./gameplay_schemas/schema.ts";
import {
...
export const Action = z.discriminatedUnion("game", [
  ...
  z.object({ game: z.literal(GameKind.Poker), action: PokerActionSchema }),
]);
export type Action = z.infer<typeof Action>;

export const State = z.discriminatedUnion("game", [
  // deno-lint-ignore no-explicit-any
  Connect4StateSchema as any,
  // deno-lint-ignore no-explicit-any
  PokerStateSchema as any,
]);
...
export const AgentSlug = z
  ...
  .refine(
    (s) => {
      ...
      if (!NameSchema.safeParse(username).success) {
        return false;
      }
      if (!NameSchema.safeParse(agentname).success) {
        return false;
      }
      return true;
    },
