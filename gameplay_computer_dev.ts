...
import { schema, GamePlayDB } from "./gameplay_schema.ts";
import { GamePlayContext, app } from "./gameplay_web.tsx";
...
dev_app.use(async (c: GamePlayContext, next) => {
  c.set("db", db);
  c.set("clerk_publishable_key", Deno.env.get("GAMEPLAY_DEV_CLERK_PUBLISHABLE_KEY")!);
